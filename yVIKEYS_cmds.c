...
yvikeys_srch_exec     (uchar *a_search, int *a_found)
{
   ...
   /*---(position to first)--------------*/
   if (s_nfind > 0)  yvikeys_srch_cursor ('[');
   /*---(complete)-----------------------*/
   DEBUG_SRCH   yLOG_exit    (__FUNCTION__);
   return rc;
}
...
yVIKEYS_srch_found   (char *a_label, int a_buf, int x, int y, int z)
{
   ...
   /*---(update current)-----------------*/
   s_cfind    = s_tfind;
   DEBUG_SRCH   yLOG_point   ("s_cfind"   , s_cfind);
   /*---(complete)-----------------------*/
   DEBUG_SRCH   yLOG_exit    (__FUNCTION__);
   return 0;
}
...
yvikeys_srch_cursor     (char a_move)
{
   ...
   /*---(defenses)-----------------------*/
   DEBUG_SRCH   yLOG_point   ("s_hfind"   , s_hfind);
   --rce;  if (s_hfind == NULL) {
      s_cfind  = NULL;
      s_ifind  = -1;
      DEBUG_SRCH   yLOG_exitr   (__FUNCTION__, rce);
   ...
   if (s_cfind == NULL) {
      s_cfind  = s_hfind;
      s_ifind  = 0;
      DEBUG_SRCH   yLOG_point   ("fixed"     , s_cfind);
   }
   DEBUG_SRCH   yLOG_value   ("s_ifind"   , s_ifind);
   /*---(handle move)--------------------*/
   switch (a_move) {
   case '[' :
      s_cfind  = s_hfind;
   ...
   if (s_cfind == NULL) {
      switch (a_move) {
      case '<' :
         DEBUG_SRCH   yLOG_note    ("bounced off head");
         s_cfind  = s_hfind;
         s_ifind  = 0;
         rc = 1;
         break;
      case '>' :
         DEBUG_SRCH   yLOG_note    ("bounced off tail");
         s_cfind  = s_tfind;
         s_ifind  = s_nfind - 1;
         rc = 1;
         break;
...
int yVIKEYS_srch_count (void) { return s_nfind; }
...
SRCH__unit              (char *a_question, char n)
{
   ...
   if      (strcmp (a_question, "results"        )   == 0) {
      ...
      x_curr = s_hfind;
      while (x_curr != NULL) {
         DEBUG_SRCH   yLOG_complex ("entry"     , "%2d %-10.10s %10p %10p %10p", c, x_curr->label, x_curr, x_curr->m_next, x_curr->m_prev);
         if (c == n) {
            DEBUG_SRCH   yLOG_point   ("found"     , x_curr);
            break;
         }
         x_curr = x_curr->m_next;
         ++c;
