...
struct cMODE_INFO {
   ...
   cchar       three       [ 5];       /* very short name                     */
   cchar       terse       [10];       /* short name                          */
   cchar       cat;                    /* category for reporting              */
   /*---(movement)----------*/
   char        allow       [25];       /* allowed mode transitions            */
   /*---(status)------------*/
   cchar       expect      [50];       /* expected prep and setup             */
   char        actual      [50];       /* actual prep and setup               */
   /*---(message)-----------*/
   cchar       desc        [50];       /* description of mode                 */
...
static tMODE_INFO  s_modes [MAX_MODES] = {
   ...
   { SMOD_MAP_REG , 's', 'y', "reg", "register"  , 1, ""                    , "5f--- p i ----- n 1---- r 0M-------- d o", "----- - - ----- - ----- - ---------- - -", "selecting specific registers for data movement"     , "regs=\"a-zA-Z-+0  pull=yYxXdD  -/+=vVcCtTsSfF  push=pPrRmMaAiIoObB  mtce=#?!g"           ,    0 },
   ...
   { UMOD_HISTORY , 's', 'y', "his", "history"   , 1, ""                    , "5---- p i ----- n 1---- r 0--------- d o", "----- - - ----- - ----- - ---------- - -", "map level undo and redo"                            , ""                                                                                        ,    0 },
   ...
   { SMOD_SRC_REG , 's', 'y', "srg", "src reg"   , 2, ""                    , "5f:-- p i ----- n ----- r 0--------- d o", "----- - - ----- - ----- - ---------- - -", "selecting specific registers for text movement"     , "regs=\"a-zA-Z-+0  pull=yYxXdD  -/+=vVcCtTsSfF  push=pPrRmMaAiIoObB  mtce=#?!g"           ,    0 },
   ...
   { MODE_GOD     , 'M', 'y', "GOD", "god"       , 4, ":\/,P9"              , "5---- p i ----- n ----- r 0--------- d o", "----- - - ----- - ----- - ---------- - -", "god-mode allowing 3D omnicient viewing"             , "linear=LnhHJjkKIioO  rotate=PpaAYytTRrwW"                                                ,    0 },
